...
twincities2.df <- twincities.df[substr(twincities.df$i,1,2) %in% c("IT") &
                              substr(twincities.df$j,1,2) %in% c("IT",'AT', "SI",
                                                              "GR", "CH"), ]
twincities2.spdf <- getLinkLayer(spdf = nuts2.spdf, df = twincities2.df[,1:2])
gradLinkLayer(spdf = twincities2.spdf, df = twincities2.df,
              spdfids = "i", spdfide = "j",
